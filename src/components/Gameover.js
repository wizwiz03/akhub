...
const Gameover = ({ score_results, score, playAgain, perfect_res }) => {
  ...
  const gameover_text = {
    ...
    perfect: 'A perfect score... Incredible! You are one of a kind. Enjoy this super rare sticker.'
  };
    ...
    <Paper elevation={8} sx={{ padding: '24px 8px', flex: 1 }}>
      <Stack alignItems='center' spacing={2}>
        ...
        <Typography variant='p' textAlign='center'>
          {perfect_res ? gameover_text['perfect'] : gameover_text[result]}
        </Typography>
        ...
        <Box component='img' alt='lost game image' sx={{ width: '256px' }}
          src={ perfect_res ? perfectImg : Object.values(stickers[result])[parseInt(Object.keys(stickers[result]).length*Math.random())]}
        />
      </Stack>
    </Paper>
  );
